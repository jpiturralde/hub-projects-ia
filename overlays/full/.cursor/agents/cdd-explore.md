...
description: >
  ...
  to perform relevamiento — transcripts, client documentation, repos, architecture files —
...
2. Explore one domain only: read its index and at most two content files. State the missing evidence before opening more.
...
Do NOT redact client deliverables in `docs/draft/` — your job is relevamiento only, not drafting.

## Engram summary (mandatory for a named change, max 250 words)
