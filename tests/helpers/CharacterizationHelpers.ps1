...
function Get-EquivalenceProjectSnapshot {
  ...
  if (Test-Path -LiteralPath $engagementPath) {
    ...
    foreach ($key in @('schemaVersion', 'stackProfile', 'requestedProfile', 'engramMcpSource', 'gentleAiScope', 'includeDrawioMcp', 'includeBacklogMcp', 'includeArchiMcp', 'requires')) {
    ...
    foreach ($key in @('schemaVersion', 'stackProfile', 'projectName', 'gentleAiScope', 'requires')) {
      if ($meta.PSObject.Properties.Name -contains $key) {
        $metadata[$key] = if ($key -eq 'requires') { ConvertTo-EquivalenceRequiresContract -Requires $meta.requires } else { $meta.$key }
      }
    }
